...
public class Solution {
    public int minSetSize(int[] arr) {
        ...
        //遍历数组，并统计每个数字的出现次数
        for (Integer ele : arr) {
            check.put(ele, check.getOrDefault(ele, 0) + 1);
        }

        //对出现次数进行降序排序
        List<Integer> list = new ArrayList<>(check.values());
        list.sort((a, b) -> b - a);
        int count = 0;
        int number = 0;

        //统计出现次数的累计，如果超过数组长度的一般，可返回结果
        for (Integer au : list) {
            number += au;
            count++;
            if(number >= length / 2) {
...
//        3.2.1) ArrayDeque
//        是一个基于数组的双端队列，和ArrayList类似，它们的底层都采用一个动态的、可重分配的Object[]数组来存储集合元素，当集合元素超出该数组的容量时，系统会在底层重　　　　　　　新分配一个Object[]数组来存储集合元素
//        3.2.2) LinkedList
//        1.2 Map
